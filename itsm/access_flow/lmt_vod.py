...
if __name__ == '__main__':
    # 用户的原始请求URI部分
    origin_params = {'query': '*', 'filter_field': 'hostname:"192.168.73.75"', 'time_range': '-20m,now'}
    # 统计请求json
    post_body = '{ "query": ' \
                '{ "method_split_result": ' \
                '{ "terms": { "field": "tag_lmt_vod_access.url" } ,' \
                '"group":{' \
                '"resp_len_stats_result":{"stats":{"field": "tag_lmt_vod_access.flow"}}' \
                '}' \
                '}' \
                ' }' \
                ' }'
    ...
    if result:
        ...
        for m in res_str["data"]["method_split_result"]["buckets"]:
            domainKey = m["key"].split("/")[2]
            # print round(m["resp_len_stats_result"]["sum"]/1000/1000/5,2)
            if domainName.__len__() == 0:

                allflow = allflow + round(m["resp_len_stats_result"]["sum"] / 1000 / 1000 / 5, 2)
                domainName.setdefault(domainKey, allflow)
                print domainName
                # domainName.[m["key"]]=m["key"]
            elif domainName.__len__() > 0 & (domainName.has_key(domainKey)):
                allflow = allflow + round(m["resp_len_stats_result"]["sum"] / 1000 / 1000 / 5, 2)
                domainName[domainKey] = allflow
            else:
                allflow = allflow + round(m["resp_len_stats_result"]["sum"] / 1000 / 1000 / 5, 2)
                domainName.setdefault(domainKey, allflow)
