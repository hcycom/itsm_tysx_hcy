...
if __name__ == '__main__':
  #用户的原始请求URI部分
  origin_params = {'query': '*', 'filter_field': 'appname:"lmt_zb_db_access"','time_range':'-5m,now'}
  #统计请求json
  post_body = '{ "query": ' \
                '{"method_split_result": { "terms": { "field": "tag_lmt_zb_db_access.domainName" } ,' \
                    '"group":{"resp_len_stats_result":{' \
                                '"stats":{"field": "tag_lmt_zb_db_access.flow"}'\
                            '}}' \
              '}}}'
  #用户请求签名的时间
  ...
  if result:
   ...
   for m in res_str["data"]["method_split_result"]["buckets"]:
     print m
     print m["key"]
     print round(m["resp_len_stats_result"]["sum"]/1024/1024/300*8,2)
